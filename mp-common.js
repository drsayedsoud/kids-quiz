...
export function saveRoomToLocal(roomCode, room) {
    ...
    localStorage.setItem('mp_qorder', /^(ordered|term1|term2|all)$/.test(settings.qOrder || '') ? settings.qOrder : 'ordered'); // shared question-order mode
...
export function clearMpState() {
    ['mp_roomCode', 'mp_isHost', 'mp_mode', 'mp_val', 'mp_round', 'mp_sync', 'mp_teams', 'mp_pick', 'mp_qstart'].forEach(k => localStorage.removeItem(k));
}
